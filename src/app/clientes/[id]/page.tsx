    ...
    <div className="min-h-screen px-4 py-8 max-w-4xl mx-auto">
      ...
      <div className="mb-6">
        ...
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 rounded-2xl bg-brand-500/20 border border-brand-500/30 flex items-center justify-center text-brand-400 font-black text-xl">
              {client.nombre.charAt(0).toUpperCase()}
            </div>
            <div>
              <h1 className="text-2xl font-black" style={{ fontFamily: 'Syne, sans-serif' }}>{client.nombre}</h1>
      ...
      {plan ? (
        <>
          ...
          <div className="glass rounded-2xl p-5 mb-6">
            <div className="flex items-start justify-between mb-2">
              <div>
                <h2 className="text-lg font-bold">{plan.titulo}</h2>
          ...
          {/* Week objective */}
          {currentWeekData && (
            <div className="bg-brand-500/10 border border-brand-500/20 rounded-xl px-4 py-3 mb-4 text-sm text-brand-300">
          ...
          <div className="grid gap-3 mb-6">
            {currentWeekData?.dias.map((dia, idx) => (
              <button
                ...
                className="glass rounded-2xl p-4 text-left hover:bg-white/[0.06] transition-all w-full"
              >
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-slate-500">⏱ {dia.duracion_min} min</span>
                    <span className="text-xs text-slate-500">{dia.ejercicios.length} ejercicios</span>
                    <span className="text-slate-600">{activeDay?.dia === dia.dia ? '▲' : '▼'}</span>
                  </div>
                </div>
          ...
          {plan.recomendaciones_nutricionales && (
            <div className="glass rounded-2xl p-5">
              <h3 className="font-bold mb-2">🥗 Recomendaciones nutricionales</h3>
        ...
        <div className="glass rounded-2xl p-8 text-center">
          ...
          <h3 className="font-bold mb-2">Sin plan activo</h3>
          <p className="text-slate-500 text-sm mb-4">Este cliente no tiene un plan generado todavía</p>
        </div>
      )}
    </div>
  )
...
function Tag({ children }: { children: React.ReactNode }) {
  return (
    <span className="text-xs px-2 py-1 glass rounded-lg text-slate-400">{children}</span>
  )
}
